...
# @@File             :  install.sh
# @@Description      :  Install configurations for neofetch
...
__run_post_install() {
  [ -d "$HOME/.local/bin" ] || mkd "$HOME/.local/bin"
  if ! __cmd_exists neofetch; then
    __download_file "https://github.com/dylanaraps/neofetch/raw/master/neofetch" "$HOME/.local/bin/neofetch"
    [ -f "$HOME/.local/bin/neofetch" ] && chmod -Rf 755 "$HOME/.local/bin/neofetch"
  fi
  return ${?:-0}
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
